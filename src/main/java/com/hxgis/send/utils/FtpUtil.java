...
public class FtpUtil {
    ...
    public boolean connect(String url, int port, String username, String password) throws IOException {
        url = url.trim();
        int firstSplitIndex = url.indexOf("/");
        String ip,directory = "";
        if(firstSplitIndex>0){
            ip = url.substring(0,firstSplitIndex);
            directory = url.substring(firstSplitIndex+1);
        }else {
            ip = url;
        }
    ...
    public void ftpEncode() throws IOException {
    ...
    public void upload(File file) throws Exception {
        if (file.isDirectory()) {
            ...
            ftpClient.storeFile(new String(file2.getName().getBytes(encoding), "iso-8859-1"), input);
            input.close();
        }
    }
    ...
    public void upload(String name, byte[] bytes) throws Exception {
        ...
        // FTP协议里面，规定文件名编码为iso-8859-1
        // 当前系统使用的Oracle的编码格式为gbk
        boolean uploaded = ftpClient.storeFile(new String(name.getBytes(encoding), "iso-8859-1"), input);
        if(!uploaded){
            throw new RuntimeException("无法上传文件");
        }
        input.close();
    }
